...
CREATE TABLE AUTHORS(ID BIGSERIAL CONSTRAINT PK_AUTHORS PRIMARY KEY, NAME VARCHAR(255));

DROP TABLE IF EXISTS GENRES;
CREATE TABLE GENRES(ID BIGSERIAL CONSTRAINT PK_GENRES PRIMARY KEY, NAME VARCHAR(255));
CREATE UNIQUE INDEX GENRE_NAME ON GENRES (NAME);

DROP TABLE IF EXISTS BOOKS;
CREATE TABLE BOOKS(ID BIGSERIAL CONSTRAINT PK_BOOKS PRIMARY KEY, NAME VARCHAR(255), AUTHOR_ID BIGINT);
...
CREATE TABLE COMMENTS(ID BIGSERIAL CONSTRAINT PK_COMMENTS PRIMARY KEY, COMMENT VARCHAR(1000) NOT NULL, AUTHOR VARCHAR(1000), CREATED_DATE TIMESTAMP, BOOK_ID BIGINT);
ALTER TABLE COMMENTS ADD CONSTRAINT COMMENTS_BOOK_FK FOREIGN KEY(BOOK_ID) REFERENCES BOOKS;

DROP TABLE IF EXISTS USER_INFO;
CREATE TABLE USER_INFO(ID BIGSERIAL CONSTRAINT PK_USER_INFO PRIMARY KEY, LOGIN VARCHAR(250) NOT NULL, PASSWORD VARCHAR(250) NOT NULL, ROLE VARCHAR(100) NOT NULL);
